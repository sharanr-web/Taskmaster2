...
-- TASKMATION DATABASE SCHEMA FOR SUPABASE
...
CREATE TABLE public.profiles (
  ...
  staff_picks_count INT DEFAULT 0,
...
CREATE TABLE public.challenges (
  ...
  principles_focus TEXT[] NOT NULL,
...
-- 4. Create Mentor Feedback Table
CREATE TABLE public.feedback (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  submission_id UUID REFERENCES public.submissions(id) ON DELETE CASCADE UNIQUE,
  mentor_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE,
  timing_score NUMERIC(3,1) CHECK (timing_score >= 1 AND timing_score <= 10),
  spacing_score NUMERIC(3,1) CHECK (spacing_score >= 1 AND spacing_score <= 10),
  arcs_score NUMERIC(3,1) CHECK (arcs_score >= 1 AND arcs_score <= 10),
  creativity_score NUMERIC(3,1) CHECK (creativity_score >= 1 AND creativity_score <= 10),
...
-- Public can view active challenges & approved submissions
...
CREATE POLICY "Users can upload their submission" ON public.submissions FOR INSERT WITH CHECK (auth.uid() = user_id);
`;
